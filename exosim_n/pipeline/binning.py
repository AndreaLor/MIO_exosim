...
#                sig_stack = np.vstack((sig_stack,sig.mean(axis=0)))
     ...
     def obtainAllen_phot(self):
        print (self.LC) 
        timestep = self.opt.exposure_time.value
        binMax = int(len(self.LC)/20 )
        sig_stack=[]
        no_stack=[]
        for binSize in range (1, binMax):
            idx = np.arange(0, len(self.LC),binSize)
            sig = (np.add.reduceat(self.LC,idx)/ binSize)[:-1]
#            noise = sig.std(axis=0) / sig.mean(axis=0)
            noise = sig.std() / self.LC_signal.mean()
            sig_stack.append(self.LC_signal.mean())
            no_stack.append(noise)
        ...
        x_fit = np.log10(self.ootAllen[0][idx:])
        y_fit = np.log10(self.ootAllen[2][idx:])             
        init = [-2.2,-0.5]
        if y_fit[0]<1:
            m_est = (y_fit[-1]-y_fit[0]) / (x_fit[-1]-x_fit[0])                        
            init = [y_fit[0],-0.1]   
        ...
        if self.opt.diagnostics == 1:
            plt.figure('Allan plots')
            plt.loglog(self.ootAllen[0], self.ootAllen[2], 'o')
            plt.plot(fit_time, fit_y_ , '--')
            plt.ylabel('noise (ppm)')
            plt.xlabel('binned time')
            plt.grid(True)
    ...
    def __init__(self, data, opt, diff, ApFactor, final_ap):
        ...
        if self.opt.pipeline.pipeline_apply_mask.val==1:
            ...
            for i in range(data.shape[2]): 
                self.binnedLC.append(data[...,i].sum())
 
        if self.opt.timeline.apply_lc.val == 1:
            self.binnedGamma = self.opt.ldc[:,0].reshape(len(self.opt.ldc[:,0]),1)[1:]
  
     
 
    def applyAperture(self):
        
        wl = self.opt.phot_wav.value
        F_ = self.opt.channel.wfno_y.val
        ApFactor = self.ApFactor
        pixSize = (self.opt.channel.detector_pixel.pixel_size.val).to(u.um).value
        ...
        ProgMax = 100    # number of dots in progress bar
        if nWLs<ProgMax:   ProgMax = nWLs   # if less than 20 points in scan, shorten bar
        print ("|" +    ProgMax*"-"    + "|     Applying photometric aperture %s"%(self.final_ap))
...
#                 print ("using fixed position due to low SNR..........")
    ...
    #    
        ...
        self.binnedLC = np.array(signal).reshape(len(signal),1)
        self.binnedWav = np.array([wl])
