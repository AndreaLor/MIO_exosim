...
def run(opt):
  ...
  exosim_msg ('exposure_time %s'%(opt.exposure_time), opt.diagnostics)
  exosim_msg ('allocated_time %s'%(opt.allocated_time) ,  opt.diagnostics)
  ...
  exosim_msg ("initial NDR time %s"%(NDR_time ),  opt.diagnostics)
   
  exosim_msg ("overheads %s %s %s"%(opt.timeline.nGND()*opt.frame_time, opt.timeline.nNDR0()*opt.frame_time, opt.timeline.nRST()*opt.frame_time),  opt.diagnostics)
 ...
 # Calcaulate exposure time (=total cycle time) and estimates how many exposures are needed
  ...
  if opt.timeline.apply_lc.val ==1:
      exosim_msg ("Since light curve is implemented, observing time is set to 2x T14",  opt.diagnostics)
  ...
  exosim_msg ("number of integrations %s"%(number_of_exposures),  opt.diagnostics)
  exosim_msg ("number of NDRs %s"%(number_of_exposures*opt.multiaccum),  opt.diagnostics)
  exosim_msg ("total observing time (hrs) %s"%((number_of_exposures*opt.exposure_time/3600).value),  opt.diagnostics)
  exosim_msg ("T14 %s"%(opt.T14),  opt.diagnostics)
  ...
  exosim_msg ("actual NDR time %s"%(opt.ndr_end_time[1]-opt.ndr_end_time[0]  ),  opt.diagnostics) 
  exosim_msg ("final exposure time %s"%(opt.exposure_time ) , opt.diagnostics) 
