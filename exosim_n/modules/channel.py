...
def run(opt):
      ...
      exosim_n_lib.sed_propagation(opt.star.sed, opt.channel_transmission)   
      ...
      exosim_n_plot('PCE', opt.diagnostics, xdata = PCE.wl, ydata = PCE.sed)    
      exosim_n_plot('Total transmission not including QE', opt.diagnostics, xdata = TotTrans.wl, ydata = TotTrans.sed)
      opt.qe_spec = qe
      opt.Re = qe.sed * (qe.wl).to(u.m)/(const.c.value * const.h.value * u.m)
      opt.star.sed.sed *= opt.Re*u.electron/u.W/u.s
      exosim_n_plot('channel star sed check', opt.diagnostics, 
                   xdata = opt.x_wav_osr, ydata=opt.star.sed.sed, marker='-' )        
      exosim_n_msg('check 1.3 - Star sed max:  %s'%(opt.star.sed.sed.max()), opt.diagnostics)    
      exosim_n_plot('Wavelength solution check -oversampled pixels', opt.diagnostics, 
                   xdata = opt.x_pix_osr, ydata =opt.x_wav_osr, marker='o-' )
      exosim_n_plot('Wavelength solution check -normal pixels', opt.diagnostics, 
      ...
      exosim_n_plot('D x wav osr check', opt.diagnostics, 
                   xdata = opt.x_pix_osr, ydata = opt.d_x_wav_osr, marker='o')
      exosim_n_plot('D x wav osr check 2', opt.diagnostics, 
                   xdata = opt.x_wav_osr, ydata = opt.d_x_wav_osr, marker='o')    
      exosim_n_plot('D x wav osr check 3', opt.diagnostics, 
                   xdata = opt.x_wav_osr, ydata = np.gradient(opt.d_x_wav_osr), marker='-')
      exosim_n_msg ("check 1.4:  %s"%(opt.star.sed.sed.max()), opt.diagnostics)   
      opt.planet_sed_original = copy.deepcopy(opt.planet.sed.sed)
      if opt.channel.is_spec==True:
          opt.planet.sed.sed  *= opt.star.sed.sed
          opt.planet.sed.sed  *= opt.d_x_wav_osr
      exosim_n_msg ("check 1.5:  %s"%(opt.star.sed.sed.max()), opt.diagnostics)                    
      opt.star.sed.sed     *= opt.d_x_wav_osr       
      exosim_n_msg ("check 2:  %s"%(opt.star.sed.sed.max()), opt.diagnostics)  
