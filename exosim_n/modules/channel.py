...
def run(opt):
      ...
      exosim_msg('channel check0 : %s'%(opt.star.sed.sed.max()), opt.diagnostics)
      ...
      exolib.sed_propagation(opt.star.sed, opt.channel_transmission)
...
#==============================================================================
      ...
      exosim_plot('channel star sed check', opt.diagnostics, 
                   xdata = opt.x_wav_osr, ydata=opt.star.sed.sed, marker='-' )     
      
      exosim_msg('check 1.3 - Star sed max:  %s'%(opt.star.sed.sed.max()), opt.diagnostics)  
            
      
      exosim_plot('Wavelength solution check -oversampled pixels', opt.diagnostics, 
                   xdata = opt.x_pix_osr, ydata =opt.x_wav_osr, marker='o-' )

      exosim_plot('Wavelength solution check -normal pixels', opt.diagnostics, 
      ...
      exosim_plot('D x wav osr check', opt.diagnostics, 
                   xdata = opt.x_pix_osr, ydata = opt.d_x_wav_osr, marker='o')
      exosim_plot('D x wav osr check 2', opt.diagnostics, 
                   xdata = opt.x_wav_osr, ydata = opt.d_x_wav_osr, marker='o')     
         
      exosim_msg ("check 1.4:  %s"%(opt.star.sed.sed.max()), opt.diagnostics)   
      ...
      exosim_msg ("check 1.5:  %s"%(opt.star.sed.sed.max()), opt.diagnostics)  
      
                     
      opt.star.sed.sed     *= opt.d_x_wav_osr
      
          
      exosim_msg ("check 2:  %s"%(opt.star.sed.sed.max()), opt.diagnostics)  
      exosim_plot('star sed check 2.0', opt.diagnostics, 
                   xdata=opt.x_wav_osr, ydata=opt.star.sed.sed , marker='-')
      
      
      
      # opt.star_sed2.rebin(opt.x_wav_osr[1::3])
      # opt.star_sed2.sed *=opt.Aeff
      # opt.star_sed2.sed *= PCE.sed[1::3] * np.gradient(opt.x_wav_osr[1::3])
      # opt.star_sed2.sed *= (qe.wl[1::3]).to(u.m)/(const.c.value * const.h.value * u.m)
      
      
      # plt.figure('sed..')
      # plt.plot(opt.x_wav_osr[1::3], opt.star_sed2.sed)
