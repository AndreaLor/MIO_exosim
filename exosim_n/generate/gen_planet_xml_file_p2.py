...
def make_planet_xml_file(opt, pl):
    ...
    for pl in pl_list:
        ...
        if cond ==1:
            ...
            nan_list =[]
            ct = -1
            #ecliptic_lat=R_p=M_p=T_p= i = e = a =P =R_s = M_s= T_s= d=Z = T14=np.nan
            dic2 = {'elat': np.nan,
                   'pl_radj': np.nan,'pl_bmassj': np.nan, 'pl_eqt':np.nan,  'pl_orbincl': np.nan, 'pl_orbeccen': np.nan, 'pl_orbsmax': np.nan,
                   'pl_orbper': np.nan ,'st_rad': np.nan,'st_mass': np.nan,'st_teff': np.nan,'sy_dist': np.nan,'st_met': np.nan, 'pl_trandur': np.nan }
            for idx in idx_list:
                ct+=1
                ...
                #ecliptic_lat=R_p=M_p=T_p= i = e = a =P =R_s = M_s= T_s= d=Z = T14=np.nan
                #variables=[ecliptic_lat,R_p,M_p,T_p,i, e, a ,P ,R_s , M_s, T_s, d,Z , T14]
                #keyNames=['elat','pl_radj','pl_bmassj','pl_eqt','pl_orbincl','pl_orbeccen','pl_orbsmax','pl_orbper',
                #    'st_rad','st_mass','st_teff','sy_dist','st_met','pl_trandur']
                #count=-1
                for key in dic2:
                 
                    try:
                        dic2[key]= data[key][idx]
                    except KeyError as error:
                        print("key %s not present in file "%(error)) 
                        continue

                
                ecliptic_lat = dic2['elat']
                R_p = dic2['pl_radj']
                M_p = dic2['pl_bmassj']
                T_p = dic2['pl_eqt']
                i = dic2['pl_orbincl']
            ...
            dic2 = {'hostname':'', 'sy_jmag':'', 'sy_kmag':'', 'elat': np.nan,
                   'pl_radj': np.nan,'pl_bmassj': np.nan, 'pl_eqt':np.nan,  'pl_orbincl': np.nan, 'pl_orbeccen': np.nan, 'pl_orbsmax': np.nan,
                   'pl_orbper': np.nan ,'st_rad': np.nan,'st_mass': np.nan,'st_teff': np.nan,'sy_dist': np.nan,'st_met': np.nan, 'pl_trandur': np.nan }
            ...
            for i in range(len(missing_params)):
                name=params[missing_params[i]]
                mp = np.array([np.nan])
                try:
                     mp = np.array( data0[name])
                except KeyError as error:
                        pass
                ...
                if len(has_param) == 0:
                    pass
                    print ("no data found for parameter: %s enter manually"%name)
