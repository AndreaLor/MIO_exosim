...
# def Psf_spectrometer_file(zfile, opt):
#     with open(zfile, 'rb') as handle:
#        dic = pickle.load(handle)
#     wl_file = dic['wl']
#     psf_file = dic['psf']
#     wav = opt.x_wav_osr
#     psf_stack = interpolate.interp1d(wl_file, psf_file, axis=2, bounds_error=False, fill_value=0.0, kind='linear')(wav)
#     for i in range(psf_stack.shape[2]):
#         psf_stack[...,i] = psf_stack[...,i]/ psf_stack[...,i].sum()    
#     psf_stack[np.isnan(psf_stack)] = 0
    
 
#     buffer = int(psf_stack.shape[1]/2)     
#     test_fp = np.zeros((opt.fpn[0]*3 , opt.fpn[1]*3 + buffer*2)    )       
#     i0 = np.array([test_fp.shape[0]/2 - psf_stack.shape[0]/2]* test_fp.shape[1]).astype(np.int)                 
#     i1 = i0 + psf_stack.shape[0]  
     
#     for k in range (0,  test_fp.shape[1]-buffer*2,1):
#         test_fp[i0[k]: i1[k] ,k:k+psf_stack.shape[1] ] += psf_stack[...,k]
#     test_fp = test_fp[:,buffer:-buffer]    
#     plt.figure('check PSF volume on fp - should be 1')
#     plt.plot(wav, test_fp.sum(axis=0)) 
...
#==============================================================================        
    ...
    xmax,ymax = np.unravel_index(np.argmax(x[...,0], axis=None), x[...,0].shape)
    if x[...,0].shape[1]> x[...,0].shape[0]:
        sub = int(-5+x[...,0].shape[0]/2)
        x = x[xmax-sub: xmax+sub, ymax-sub: ymax+sub]
    elif x[...,0].shape[1]< x[...,0].shape[0]:
        sub = int(-5+x[...,0].shape[1]/2)
        x = x[xmax-sub: xmax+sub, ymax-sub: ymax+sub]     
    for i in range (x.shape[2]):
        x[...,i] = x[...,i]/  x[...,i].sum()        
    img_array = x
...
#==============================================================================
  ...
  exosim_n_msg ("diffusion length in IPRF %s"%(lx), opt.diagnostics)
...
def pointing_jitter(opt):
  ...
  exosim_n_msg ("Frame OSF (number of jitter frames per NDR %s"%(osf) , opt.diagnostics)
  ...
  exosim_n_msg ("interpolation of PSD done", opt.diagnostics)
  # import matplotlib.pyplot as plt
  # plt.figure(33)
  ...
  exosim_n_msg ("jitter RMS in mas %s %s"%(np.std (yaw_jit.value)*3600*1000, np.std(pitch_jit.value)*3600*1000) , opt.diagnostics)
