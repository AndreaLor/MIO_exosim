...
def apply_read_noise(signal, opt):   
    if opt.simulation.sim_full_ramps.val == 0 and opt.simulation.sim_use_UTR_noise_correction.val == 1:             
        ...
        signal = read_noise_UTR_correction(n, signal.value , opt.channel.detector_pixel.sigma_ro.val.value)
        signal = signal*u.electron    
    else:    
        signal = read_noise(signal.value, opt.channel.detector_pixel.sigma_ro.val.value)
        signal = signal*u.electron
    return signal
...
def read_noise (signal, sigma_ro):
    signal = signal + np.random.normal(scale = sigma_ro, size = signal.shape)  
    return signal
...
def simulate_jitter(opt):       
    exosim_n_msg ("using jittered array" , opt.diagnostics)
    opt.jitter_psd_file = '%s/exosim_n/systems/%s/Pointing_models/%s'%(opt.exosim_n_path, opt.system, opt.simulation.sim_pointing_model.val)
    exosim_n_msg ("psd file %s"%(opt.jitter_psd_file), opt.diagnostics)
    exosim_n_msg ("running jitter code", opt.diagnostics)
    try:
        opt.use_external_jitter
    except AttributeError:
        opt.use_external_jitter=0
    if opt.use_external_jitter==0:
        exosim_n_msg ("generating new jitter timeline...",  opt.diagnostics)
        opt.yaw_jitter, opt.pitch_jitter, opt.frame_osf = exosim_n_lib.pointing_jitter(opt)
    elif opt.use_external_jitter==1:
        exosim_n_msg ("using external jitter timeline...", opt.diagnostics)
        opt.yaw_jitter, opt.pitch_jitter, opt.frame_osf = opt.input_yaw_jitter, opt.input_pitch_jitter, opt._input_frame_osf 
    exosim_n_msg ("RMS jitter %s %s"%(np.std(opt.yaw_jitter), np.std(opt.pitch_jitter)  ) , opt.diagnostics)     
    ...
    exosim_n_msg ("RMS jitter in pixel units x axis %s"%(np.std(jitter_x)/opt.osf), opt.diagnostics)
    exosim_n_msg ("RMS jitter in pixel units y axis %s"%(np.std(jitter_y)/opt.osf), opt.diagnostics)
    ...
    jitter_x *= mag
    jitter_y *= mag  
    exosim_n_msg ("spatial osf %s"%(osf) , opt.diagnostics)    
    ...
    exosim_n_msg('Applying jitter', 1)
    ...
    if cond == 1: 
        exosim_n_msg('Splitting array for jitter code', 1)
        overlap = 5*osf
        x_size= int(5000/ (fp.shape[0]/osf) ) *osf
        n_div = int(fp.shape[1]/x_size)
        exosim_n_msg(f'Number of divisions {n_div}', opt.diagnostics)
        ct = 0
        ...
        exosim_n_msg(f'noise stack shape after split jitter code {div_noise_stack.shape}', opt.diagnostics)  
        noise  = div_noise_stack                  
    qq = opt.frames_per_ndr* fp_units*opt.frame_time
    opt.signal = noise*qq 
    exosim_n_msg('Time to run jitter code %s'%(time.time() - aa), opt.diagnostics) 
    return opt 
...
def jitterless(opt):
   exosim_n_msg ("using jitterless array", opt.diagnostics)
...
def obtain_signal_only(opt):
   exosim_n_msg ("generating seperate noiseless signal array",  opt.diagnostics  )
