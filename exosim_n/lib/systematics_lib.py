...
def gen_prnu_grid(opt):
    ...
    if opt.noise.ApplyRandomPRNU.val == 1:
        ...
        exosim_n_msg ("RANDOM PRNU GRID SELECTED...",  opt.diagnostics)
    else:
        ...
        exosim_n_msg ("PRNU GRID SELECTED FROM FILE...", opt.diagnostics)     
    return opt
...
def gen_systematic_grid(opt):
    if opt.simulation.sim_use_systematic_model.val == 1: 
        ...
        syst_grid_folder = '%s/exosim_n/data/systematic_models/%s'%(opt.exosim_n_path, syst_grid_name)
        ...
        if os.path.exists('%s/syst.txt'%(syst_grid_folder)):        
            ...
            exosim_n_msg('ERROR Systematics module 1: No systematic files found')
            sys.exit()
        ...
        for i in range (len(wl)):
            if i == 0:
                plt.plot(time, syst_grid[i], 'r-', alpha = 0.5, markersize = 1,label = 'interpolated to simulator grid')
            else:
